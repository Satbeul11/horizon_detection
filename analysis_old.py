...
# ✅ 추가된 시각화 함수: ROI 영역 회색 칠하기 + 에지 포인트 찍기
def draw_roi_visuals(vis, debug_info):
    ...
    if roi_y1 > roi_y0:
        overlay = vis.copy()
        # (B, G, R) = (128, 128, 128) 회색
        cv2.rectangle(overlay, (0, roi_y0), (w, roi_y1), (128, 128, 128), -1)
        
        alpha = 0.3  # 투명도 (0.0 ~ 1.0)
        vis = cv2.addWeighted(overlay, alpha, vis, 1 - alpha, 0)

    # 2. Canny Edge 후보점 표시 (Cyan 색상)
    if edge_map is not None:
        # edge_map은 ROI 영역 기준이므로 y좌표에 roi_y0를 더해야 원본 좌표가 됨
        ys, xs = np.where(edge_map > 0)
        
        # 점이 너무 많으면 느려질 수 있으므로, 픽셀 직접 조작 혹은 circle 사용
        # 잘 보이게 하려면 Cyan (255, 255, 0) 또는 Yellow (0, 255, 255)
        point_color = (255, 255, 0) # Cyan
        
        # 좌표 변환: ROI 내부 y -> 전체 이미지 y
        ys_global = ys + roi_y0
        
        # 화면 밖으로 나가는 것 방지
        valid_mask = (ys_global < h) & (xs < w)
        vis[ys_global[valid_mask], xs[valid_mask]] = point_color
        
        # 점을 좀 더 두껍게 찍고 싶다면 아래 for문 사용 (속도는 약간 느려짐)
        # for x, y in zip(xs, ys_global):
